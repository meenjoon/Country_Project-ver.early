...
import com.example.country_projectexample.databinding.ActivityCountriesDetailBinding
...
class CountriesDetail : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        /*
            WebView 사용을 위한 설정
         */
        binding.imageGoogleMap.settings.javaScriptEnabled = true
        binding.imageGoogleMap.webViewClient = WebViewClient()
        binding.imageGoogleMap.webChromeClient = WebChromeClient()


        binding.korNameDetail.text = korName_Common //클릭시 해당 리사이클러뷰의 값 korName_Common을 intent로 받은 후 korName_Detail에 넣어준다.
        binding.enNameOfficialDetail.text = enName_Official //클릭시 해당 리사이클러뷰의 enName_Official을 intent로 받은 후 enName_Official에 넣어준다.
        Glide.with(this) //클릭시 해당 리사이클러뷰의 flag를 intent로 받은 후 Glide를 통해 country_Flag_Detail에 넣어준다.
            .load(flag)
            .into(binding.countryFlagDetail)
            ...
            override fun onResponse( //restrofit2 통신 성공했을 때(데이터 가져옴)
                ...
                body?.let { it ->
                    ...
                    it.forEach {
                        ...
                        val cca3 = it.cca3 //activity_countries_detail.xml 사용 할 cioc(국가코드) 변수
                        ...
                        if(it.capital==null) {
                            ...
                            binding.capitalInputDetail.text = capital //capital_Input_Detail이라는 TextView와 capital 변수 연결
                        }
                        ...
                        if (it.currencies==null) {
                            binding.currencyInputDetail.text = "" //currency_Input_Detail이라는 TextView와 currency 변수 연결
                        } else {
                            ...
                            binding.currencyInputDetail.text = currency //currency_Input_Detail이라는 TextView와 currency 변수 연결
                        }

                        binding.nativeNameCommonInputDetail.text = nativeName_Common //nativeNameCommon_Input_Detail이라는 TextView와 nativeName_Common 변수 연결
                        binding.cca3InputDetail.text = cca3 //cioc_Input_Detail이라는 TextView와 cioc 변수 연결
                        binding.continentInputDetail.text = continent //continent_Input_Detail이라는 TextView와 continent 변수 연결
                        binding.populationInputDetail.text = population.toString() //population_Input_Detail이라는 TextView와 population 변수 연결
                        it.maps?.openStreetMaps.toString().let { googleMapUrl -> binding.imageGoogleMap.loadUrl(googleMapUrl) } //webView에 구글 지도 넣기

                        Log.d("language:", it.languages.toString())
                        Log.d("language", "$nativeNameTolanguage")
                        Log.d("language","${it.languages.toString().indexOf(nativeNameTolanguage)}")



                        //language 데이터 정제
                       if(it.languages.toString().isEmpty()) {
                            binding.languageInputDetail.text = ""
                        }
                        else if(it.languages.toString().indexOf(nativeNameTolanguage) !=1) {
                            ...
                            binding.languageInputDetail.text = languageSubstring
                        }
        ...
        binding.backBtnDetail.setOnClickListener{ //backBtn_Detail 클릭했을 시 뒤로가기
            finish()
        }
